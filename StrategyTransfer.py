...
parser.add_argument('--architecture', type=str, default="stacked", help='Model architecture')
...
meta_features_map = {"features": {"EFs": {"FEATURES_PATH": config["SIMULATION_EFs_PATH"], "REVIEW_DIM": 37},
                     ...
                     "architecture": {"LSTM": {"use_user_vector": True},
                                      ...
                                      "stacked": {"use_user_vector": True},
                                      "concatenated": {"use_user_vector": True},
                                      "complex_hybrid": {"use_user_vector": True}}
                     }
...
elif config["architecture"] == "stacked":
    env_model = environments.Stacked_env.StackedEnvironment(env_name, config=config)
elif config["architecture"] == "concatenated":
    env_model = environments.Concatenated_env.ConcatenatedEnvironment(env_name, config=config)
elif config["architecture"] == "complex_hybrid":
    env_model = environments.ComplexHybrid_env.ComplexHybridEnvironment(env_name, config=config)
